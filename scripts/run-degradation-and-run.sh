...
# Compose files for many_users degradation-and-run pipeline
BASE_COMPOSE="-f docker-compose.yml -f docker-compose.degradation-and-run.yml"
...
echo "Running degradation-and-run ${ITERATIONS} times (many_users)"
...
while [ "${i}" -le "${ITERATIONS}" ]; do
  ...
  # Run degradation-and-run and collector with full dependency stack
  docker compose ${BASE_COMPOSE} up --build --force-recreate --abort-on-container-exit degradation-and-run-runner metrics-collector || true
